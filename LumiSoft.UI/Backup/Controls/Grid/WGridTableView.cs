...
using LumiSoft.UI.Controls.Grid.Editors;
...
namespace LumiSoft.UI.Controls.Grid
{
	...
	{		
	    ...
	    private void OnDataViewChanged()
	    {
		    ...
		    m_pActiveRow    = null;
            // Don't change active column, it may cause view horizontal scrolling.
		    //m_pActiveColumn = null;
    		
		    // Construct row info holder
		    for(int i=0;i<m_pDataSource.Count;i++){
			    m_pRowInfos.Add(new _RowInfo(this,m_pDataSource[i]));
		    }
            if(m_pRowInfos.Count > 0){
                m_pActiveRow = m_pRowInfos[0];
		...
		private void PaintHeader(Graphics g)
		{
			...
			foreach(WGridColumn col in this.Columns.VisibleColumns){				
				...
				if(col.Bounds.Right > -1 &&  col.Bounds.Left < RowsRect.Right){
					...
					for(int c=0;c<sortList.Length;c++){
						if(sortList[c].Split(' ')[0].Equals(col.MappingName.ToLower())){
							if(sortList[c].IndexOf("desc") > -1){
								LumiSoft.UI.Controls.Paint.DrawTriangle(g,Color.Gray,new Rectangle((int)col.Bounds.Right - 13,col.Bounds.Y + 7,9,7),Direction.Down);
							}
							else{
								LumiSoft.UI.Controls.Paint.DrawTriangle(g,Color.Gray,new Rectangle((int)col.Bounds.Right - 13,col.Bounds.Y + 7,9,7),Direction.Up);
							}
							break;
						}					
					}
					...
					// Draw text
					LumiSoft.UI.Controls.Paint.DrawText(g,Color.Black,col.Font,col.Text,col.Bounds,HorizontalAlignment.Center);
				...
				if(col.Bounds.Right > -1 && col.Bounds.Left < RowsRect.Right){
					...
					switch(col.FooterType)
					{
						...
						case WGridViewFooterType.Sum:
							...
							if(col.CellTextFormat.StartsWith("f")){								
								try{									
					...
					if(footerText.Length > 0){
						// Draw text
						LumiSoft.UI.Controls.Paint.DrawText(g,Color.Black,col.Font,footerText,new Rectangle(col.Bounds.Left + 4,FooterRect.Y + 3,col.Bounds.Width - 8,FooterRect.Height - 4),col.FooterTextAlign);
		...
		private void PaintRow(Graphics g,_RowInfo rowInfo)
		{
			...
			foreach(WGridColumn col in this.Columns.VisibleColumns){
				...
				if(col.Bounds.Right > -1 && col.Bounds.Left < RowsRect.Right){
					...
					col.Editor.PaintDefault(g,rowInfo.GetValue(col),new Rectangle(col.Bounds.Left + 2,rowInfo.getY,col.Bounds.Width - 4,rowInfo.RowHeight),cellSelected,col);
			...
			if(ShowRowIndicators){
				...
				if(activeRow){
					LumiSoft.UI.Controls.Paint.DrawTriangle(g,Color.Black,new Rectangle(Bounds.X + 8,rowInfo.Bounds.Y + 5,5,9),Direction.Right);
				}
			}
			//----------------------------------------------------------------------------------------------------//
		...
		{            
			...
			if(g != null){
				Paint(g);
				m_pGrid.Invalidate(this.Bounds);
			}
        }
        ...
        internal void Process_keyPressed(KeyEventArgs e)
		{
			...
			if(e.KeyCode == Keys.Enter){			
				...
				if(m_pColumns.IndexOf(m_pActiveColumn) < m_pColumns.Count - 1){
					SelectRow(m_pActiveRow,(WGridColumn)m_pColumns[m_pColumns.IndexOf(m_pActiveColumn) + 1],false);
				}
			}
        ...
        internal void Process_mouseClicked(MouseEventArgs e) 
		{
			...
			// See if column sorting
				...
				if(mouseCol != null && m_pDataSource != null && mouseCol.AllowSort){
					string sort = m_pDataSource.Sort.ToLower();				
					if(sort.StartsWith(mouseCol.MappingName.ToLower() + " asc")){
        ...
        internal void Process_mouseMoved(MouseEventArgs e)
		{
			...
			WGridColumn mouseCol = GetColumnFromPoint(new Point(e.X,e.Y));
			if(mouseCol != null){
				if(e.X > mouseCol.Bounds.Right - 5 && e.X < mouseCol.Bounds.Right - 2){
		...
		{	
			if(m_pDragWindow == null){
				...
				if(mouseCol != null && e.X > mouseCol.Bounds.Right - 7 && e.X <= mouseCol.Bounds.Right){
				...
				//--- If column drag (reorder)
					...
					// Paint text
					LumiSoft.UI.Controls.Paint.DrawText(g,Color.Black,mouseCol.Font,mouseCol.Text,new Rectangle(0,0,mouseCol.Bounds.Width,mouseCol.Bounds.Height),HorizontalAlignment.Center);
		...
		{		
			...
			for(int i=0;i<m_pGrid.m_pActiveViews.Count;i++){
				WGridTableView view = (WGridTableView)m_pGrid.m_pActiveViews[i];
			...
			for(int i=m_FirstVisibleRow;i<m_pRowInfos.Count;i++){
				...
				if(rowStartY > rowMaxAllowedY){
					...
					rowInfo.Bounds = new Rectangle(m_ColumnsStartX,rowStartY,colX - m_ColumnsStartX,rowHeight);
				
					// Update sub grid size,location
					if(rowInfo.DetailView != null){
						int maxHeight = (int)RowsRect.Bottom - rowStartY - rowInfo.RowHeight;
			...
			if(vtScrollbarVisible){
				for(int i=m_FirstVisibleRow;i<=m_LastVisibleRow;i++){
					_RowInfo rowInfo = (_RowInfo)m_pRowInfos[i];
        ...
        internal void AutoHeight()
		{
			...
			_RowInfo _RowInfo = (_RowInfo)m_pRowInfos[m_pRowInfos.Count - 1];
			if(m_FirstVisibleRow == 0 && m_pRowInfos.Count - 1 == m_LastVisibleRow){
				int height = RowsRect.Bottom - _RowInfo.Bounds.Y - _RowInfo.RowHeight;
				if(height > 0 && height < m_pBounds.Height){
					this.setBounds(new Rectangle(m_pBounds.X,m_pBounds.Y,m_pBounds.Width,m_pBounds.Height - height + 2));
				}
			}
        }
		...
		private void SelectRow(_RowInfo _RowInfo,WGridColumn col,bool clearSelection)
		{
			...
			if(m_pGrid.m_pFocusedView != null && !m_pGrid.m_pFocusedView.Equals(this)){
				m_pGrid.m_pFocusedView.HideEditor();
				m_pGrid.m_pFocusedView.ClearSelection();
			}
		...
		{
			SelectRow((_RowInfo)m_pRowInfos[m_FirstVisibleRow],m_pActiveColumn,true);
        }
		...
		private void SelectNextRow(_RowInfo row)
		{
			if(m_pRowInfos.IndexOf(row) < m_pRowInfos.Count - 1){
				SelectRow((_RowInfo)m_pRowInfos[m_pRowInfos.IndexOf(row) + 1],m_pActiveColumn,true);
			}
        }
		...
		/// </summary>
		/// <param name="col"></param>
		private void ShowEditor(WGridColumn col)
		{
			HideEditor();
		...
		private void HideEditor()
		{
			...
			if(m_pActiveEditor != null){
				if(m_pActiveEditor.IsModified){ // TODO: call validate cell
		...
		private bool IsActiveRowModified()
		{
			...
			if(m_ActiveRowValues != null && m_pActiveRow != null){			
				for(int i=0;i<m_ActiveRowValues.Length;i++){
					if(m_ActiveRowValues[i] == null || m_pActiveRow.RowSource[i] == null){
						if(m_ActiveRowValues[i] == null && m_pActiveRow.RowSource[i] != null){
							return true;
						}
					}
					else if(!m_ActiveRowValues[i].Equals(m_pActiveRow.RowSource[i])){
						return true;
					}
				}
			}
			...
			if(m_pActiveRow != null && !m_pActiveRow.RowSource.IsNew){
				...
				if(m_pGrid.RaiseDeleteRow(m_pActiveRow.RowSource)){
					...
					if(index < m_pRowInfos.Count){
						newActiveRow = (_RowInfo)m_pRowInfos[index + 1];
					}
					else if(index > 0){
						newActiveRow = (_RowInfo)m_pRowInfos[index - 1];
					}			
		...
		public void MoveNext()
		{
			...
			if(m_pActiveRow == null && m_pRowInfos.Count > 0 && Columns.VisibleColumns.Length > 0){
				SelectRow((_RowInfo)m_pRowInfos[0],Columns.VisibleColumns[0],true);
				return;
			}
			...
			// If view is editable and current cell is modified, call validate cell value event.
			if(m_pActiveEditor != null && m_pActiveEditor.IsModified && !m_pGrid.RaiseValidateCellValue(m_pActiveColumn,m_pActiveRow.RowSource,m_pActiveEditor.EditValue)){
				return;
			}
			HideEditor();
			...
			if(activeColIndex < m_pColumns.Count - 1){
				WGridColumn column = (WGridColumn)m_pColumns[activeColIndex + 1];
				...
				if(!column.Visible || (AllowEdit && !column.AllowEdit)){
					...
					while(m_pColumns.IndexOf(column) < m_pColumns.Count - 1){
						column = (WGridColumn)m_pColumns[m_pColumns.IndexOf(column) + 1];						
		...
		/// </summary>
		/// <param name="_RowInfo"></param>
		internal void ToggleRow(_RowInfo _RowInfo)
		{
			if(_RowInfo.DetailView == null){
				ExpandRow(_RowInfo);
			}
			else{
				CollapseRow(_RowInfo);
			}
        }
		...
		{
			// Append new item row
			if(AllowEdit && AllowAppendNewRow){
			...
			set{
				...
				if(value == null){
					try{
						...
						value = ds.Tables.Add("empty").DefaultView;
					}
					catch(Exception x){				
					}
				}
                ...
                // Remove old dataview chaged listener.
                if(value != null){
                    value.ListChanged -= new System.ComponentModel.ListChangedEventHandler(m_pDataSource_ListChanged);
                }
		...
		public DataRowView[] SelectedRows
		{
			get{
				...
				for(int i=0;i<m_pSelectedRows.Count;i++){
					retVal[i] = ((_RowInfo)m_pSelectedRows[i]).RowSource;
				}
